...
def process_prompt():
    ...
    try:
        ...
        modelo_imagem = request.form.get('image_model', '').strip()  # Removido os parênteses de .strip
        
        if not unified_prompt or not modelo_imagem:
            flash('Erro: Todos os campos são obrigatórios')
            return redirect(url_for('index'))
            
        result_type, result = prompt_processor.process_prompt(unified_prompt, modelo_imagem)
        
        if result_type == 'image':
            return redirect(url_for('index',
                                  result=result,  # Adicionado o result que estava faltando
                                  result_type=result_type,
                                  image_name=result))
        else:
            return redirect(url_for('index',
    ...
    except ValueError as e:  # Adicionado tratamento específico para ValueError
